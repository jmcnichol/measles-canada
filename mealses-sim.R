...
int.8000.95 <- measles.sim(0.95,1000,T)
int.8000.90 <- measles.sim(0.90,1000,T)
int.8000.85 <- measles.sim(0.85,1000,T)
int.8000.80 <- measles.sim(0.80,1000,T)
int.8000.75 <- measles.sim(0.75,1000,T)
int.8000.70 <- measles.sim(0.70,1000,T)
int.8000.65 <- measles.sim(0.65,1000,T)

# manipulate the sim output to get outbreak size and symptomatic -- yes, i change the file name everytime, quick and dirty ;) 

library(parallel)

tdata = mclapply(int.1000.95.5,convtime,mc.cores=4)
...
tdata = mclapply(int.1000.90.5,convtime,mc.cores=4) 
tbigdf = bind_rows(tdata, .id="simnum") 
...
tdata = mclapply(int.1000.85.5,convtime,mc.cores=4) 
tbigdf = bind_rows(tdata, .id="simnum") 
...
tdata = mclapply(int.1000.80.5,convtime,mc.cores=4) 
tbigdf = bind_rows(tdata, .id="simnum") 
...
tdata = mclapply(int.1000.75.5,convtime,mc.cores=4) 
tbigdf = bind_rows(tdata, .id="simnum") 
...
tdata = mclapply(int.1000.70.5,convtime,mc.cores=4) 
tbigdf = bind_rows(tdata, .id="simnum") 
...
tdata = mclapply(int.1000.65.5,convtime,mc.cores=4) 
inctdata <- bind_rows(mclapply(tdata, addincidence,mc.cores=4), .id="simnum") 
outsizes7 <- inctdata %>% group_by(simnum) %>% summarise(size = sum(incid)) 
outsizes7 <- data.frame(outsizes7, vax = as.factor(rep(0.65)))

outsizes.1000.int.5 <- rbind(outsizes1,outsizes2,outsizes3, outsizes4, outsizes5, outsizes6, outsizes7)
sumdat.int.1000.5 <- rbind(sumdata1,sumdata2,sumdata3,sumdata4,sumdata5,sumdata6)
